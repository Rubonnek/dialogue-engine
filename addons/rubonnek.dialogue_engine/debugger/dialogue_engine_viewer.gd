...
@onready var m_dialogue_engine_viewer_input_blocker_center_container : CenterContainer = find_child("DialogueEngineViewerInputBlockerCenterContainer")
@onready var m_dialogue_engine_viewer_input_blocker_hint_rich_text_label : RichTextLabel = find_child("DialogueEngineViewerInputBlockerHintRichTextLabel")
@onready var m_default_input_blocker_hint_text : String = m_dialogue_engine_viewer_input_blocker_hint_rich_text_label.get_text()
...
func __on_dialogue_selection_line_edit_text_changed(p_filter : String) -> void:
	...
	for child : TreeItem in root.get_children():
		if child.is_visible():
			m_dialogue_engine_viewer_engine_selection_tree.set_selected(child, column) # emits item_selected signal
...
func __on_tree_item_selected() -> void:
	...
	if is_instance_valid(selected_tree_item):
		if m_dialogue_engine_viewer_input_blocker_center_container.is_visible():
...
func __clear_graph_node_highlights_if_needed(p_dialogue_engine_id : int) -> void:
	if __is_selected_tree_item_related_to_dialogue_engine(p_dialogue_engine_id):
		for child : Node in m_dialogue_engine_viewer_graph_edit.get_children():
...
func __refresh_graph_for_currently_selected_item() -> void:
	...
	for target_node_name : String in node_string_ids:
		...
		if not found_incoming_connection_to_first_slot:
			var graph_node : GraphNode = m_dialogue_engine_viewer_graph_edit.get_node(NodePath(target_node_name))
	...
	m_dialogue_engine_viewer_graph_edit.arrange_nodes()


func __cleanup_graph_edit() -> void:
	m_dialogue_engine_viewer_graph_edit.clear_connections()
	for child_node : Node in m_dialogue_engine_viewer_graph_edit.get_children():
